...
df = df.dropna(subset=['LOCATION', 'PRICE', 'CITY', 'LOT SIZE', 'BEDS', 'BATHS', 'DAYS ON MARKET', 'SQUARE FEET'])
df['HOA/MONTH'] = df['HOA/MONTH'].fillna(0)
mapper = cm.ScalarMappable()
mapper.set_array(df['PRICE'])
def distrColor():
    plt.scatter(df.LATITUDE, df.LONGITUDE, alpha=.5, c = df['PRICE'], cmap=plt.get_cmap('jet'), s = 6)
    plt.colorbar()
...
def beds():
    ...
    locs, labels = plt.yticks()
    plt.yticks(locs, ['$0', '$100,000', '$200,000', '$300,000', '$400,000', '$500,000','$600,000'])
...
def bedsVsBaths():
    ...
    plt.xlim(0)
    locs, labels = plt.yticks()
    plt.yticks(locs, ['$0','$100,000','$200,000','$300,000','$400,000','$500,000','$600,000','$700,000'])
